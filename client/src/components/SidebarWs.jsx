      ...
      >
        ...
        {isOpen ? (
          <div className="flex flex-col items-center w-full">
            ...
            <div className="mt-6 w-46 flex flex-col gap-3">
              {menuItems.map((item, index) => (
                ...
                >
                  <img className="w-5 h-5 mr-3" src={item.icon} alt={`${item.label} Icon`} />
                  {item.label}
                </button>
              ))}
            </div>
            ...
            <div className="absolute bottom-18 w-51 px-2">
              <div className="w-full bg-purple-100 rounded-lg p-2 flex items-center shadow-lg">
                <img src="https://placehold.co/40x40" alt="Admin" className="w-10 h-10 rounded-full" />
          ...
          <div className="flex flex-col items-center gap-8 mt-53">
            {menuItems.map((item, index) => (
              <img key={index} className="w-6 h-6 cursor-pointer" src={item.icon} alt={item.label} />
            ))}
          </div>
        )}
      </div>
